function check_dependencies {
  ...
  $npm_missing="Node.js/npm was not found. Please make sure you have installed Node.js version 16.0 or higher along with npm correctly. You can download Nodejs from here: https://nodejs.org/en/download"
    ...
    if (Get-Command python3 -ErrorAction SilentlyContinue) {
      if (!(python3 -c "import sys; exit(0) if sys.version_info >= (3,10) else exit(1)" 2>&1).Count -eq 0) {
        return "$py_missing"
        exit 1
      }
    } else {
    ...
    if (!(python3 -m pip --version 2-ge$null)) {
      return "$pip_missing"
      exit 1
  }

    $MINIMUM_PIP_VERSION = "20.0"
    $PIP_VERSION = (python3 -m pip --version | Select-String -Pattern '\d+(\.\d+)+').Matches.Value
    ...
    # $REDIS_CMD = Get-Command redis-cli -ErrorAction SilentlyContinue

    # if (!$REDIS_CMD) {
    #     return "Redis is not installed on your machine. Please install it to run this project. Installation guidance can be found here: https://redis.io/docs/getting-started/installation/"
    #     exit 1
    # }
    return ''
}
