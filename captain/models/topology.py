...
class Topology:
    ...
    def process_worker_response(
            self,
            finished_job_fetch: JobSuccess | JobFailure) -> list[str] | None:
        """
        ...
        if isinstance(finished_job_fetch, JobFailure):
            self.process_job_result(job_id=finished_job_fetch.node_id,
                                    job_result=None,
                                    success=False)
        ...
        # handle successful job
            ...
            logger.debug(
                f"{finished_job_fetch.node_id} finished at {time.time()}")
            return self.handle_finished_job(
                finished_job_fetch, return_new_jobs=True)  # return new jobs
    ...
    def run_job(self, job_id: str, task_queue: Queue[Any]):
        ...
        previous_jobs = self.get_job_dependencies_with_label(job_id,
                                                             original=True)
    ...
    def handle_finished_job(self,
        ...
        if self.cancelled:
            logger.debug(
                "Received job, but skipping since topology is cancelled")
            return
        ...
        logger.debug(
            f"job {self.get_label(job_id)} is done and has been received.")
    ...
    def process_job_result(self, job_id: str,
                           job_result: dict[str, Any] | None, success: bool):
        """
        ...
        logger.debug("After removing edges of node, next nodes are: " +
    ...
    def remove_edges_and_get_next(self,
                                  job_id: str,
    ...
    def restart(self, job_id: str):
        logger.debug(
            f" *** restarting job: {self.get_label(job_id, original=True)}")
        if self.loop_nodes:
            self.loop_nodes.pop()
    ...
    def get_edges_by_label(self, job_id: str,
                           label: str) -> list[tuple[str, Any, Any]]:
        edges = self.working_graph.edges(job_id, data=True)
        edges = [(s, t, data) for (s, t, data) in edges
                 if data.get("label", "") == label]
        return edges

    def get_job_dependencies_with_label(self,
                                        job_id: str,
                                        original: bool = True
                                        ) -> list[dict[str, str]]:
        graph = self.get_graph(original)
    ...
    def get_input_info(self,
                       source_job_id: str,
                       target_job_id: str,
                       original: bool = False) -> list[tuple[str, bool]]:
        graph = self.get_graph(original)
