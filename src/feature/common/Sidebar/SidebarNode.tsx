...
const iconMap = {
  NUMPY: <NumpySvg className="w-8 h-8" />,
  SCIPY: <ScipySvg className="w-8 h-8" />,
};
    ...
    <div key={node.name}>
      ...
      >
        ...
        {searchMatches?.map((command) => (
          <button
            key={command.key}
            className={`flex justify-between items-center w-11/12 px-2 py-2.5 max-h-10 mb-3 text-${color} bg-${color}/5 border border-${color} font-mono`}
            onClick={() => {
              if (query !== "") {
                sendEventToMix(
                  "Node Searched",
